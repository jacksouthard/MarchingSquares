...
public class Map : MonoBehaviour
{
    ...
    void FillTile (Vector2Int pos) {
        ...
        int roomKey = Nodes[pos.x, pos.y];
        if (roomKey >= 0) {
            // this tile belongs to a room
            rooms[roomKey].tiles.Remove(pos);
            if (rooms[roomKey].tiles.Count == 0) {
                // the room has no remaining times
                RemoveRoom(roomKey);
            }
        }

        SetNodeFilled(pos, true);
    }

    void EmptyTile (Vector2Int pos) {
        SetNodeFilled(pos, false);
    ...
    void ApplyAlteration (ref NodeAlteration alteration) {
        ...
        mapRenderer.Recalculate(startX, endX, startY, endY);
        mapCollider.Recalculate();
    }
        ...
        foreach (var tile in newlyEmptyTiles) {
            ...
            if (!GetNodeAssignedToRoom(tile)) {
                ...
                while (true) {
                    ...
                    FloodTile(floodQueTiles[0] + Vector2Int.up, ref newRoomTiles, ref floodQueTiles, ref connectedRoomKeys);
                    FloodTile(floodQueTiles[0] + Vector2Int.right, ref newRoomTiles, ref floodQueTiles, ref connectedRoomKeys);
                    FloodTile(floodQueTiles[0] + Vector2Int.down, ref newRoomTiles, ref floodQueTiles, ref connectedRoomKeys);
                    FloodTile(floodQueTiles[0] + Vector2Int.left, ref newRoomTiles, ref floodQueTiles, ref connectedRoomKeys);
                ...
                if (connectedRoomKeys.Count == 0) {
                    ...
                    foreach (var connectedRoomKey in connectedRoomKeys) {
                        int newCount = rooms[connectedRoomKey].tiles.Count;
                    ...
                    foreach (var connectedRoomKey in connectedRoomKeys) {
                        if (connectedRoomKey != roomKey) { // ignore the room we are merging into
                            foreach (var roomTile in rooms[connectedRoomKey].tiles) {
                                newRoomTiles.Add(roomTile);
                            }
                            // then delete the room
                            RemoveRoom(connectedRoomKey);
                        }
                    }

                    // update the edges of the largest room
                    // we will only be removing edge tiles, bc you cannot have more than we started
                    for (int i = rooms[roomKey].edgeTiles.Count - 1; i >= 0; i--) {
                        edgeRecalculations++;
                        if (!ShouldBeEdge(rooms[roomKey].edgeTiles[i])) rooms[roomKey].edgeTiles.RemoveAt(i);
                    }
                }

                foreach (var newTile in newRoomTiles) {
                    // recalculate edges
                    bool edge = ShouldBeEdge(newTile);
                    AssignNodeToRoom(newTile, roomKey, edge);
                }

                // now calculate the edge points based on the edge tiles
                List<Vector2Int> remainingEdgeTiles = new List<Vector2Int>(rooms[roomKey].edgeTiles);
                ...
                ReassignEdgePointsToRoom(roomKey, allEdges);
            }
        }
    ...
    void FloodTile (Vector2Int tilePos, ref List<Vector2Int> newRoomTiles, ref List<Vector2Int> floodQueTiles, ref List<int> connectedRoomKeys) {
        // make sure the tile is in the map. If its not do nothing
        if (!IsNodeInMap(tilePos)) return;
        ...
        int tileID = Nodes[tilePos.x, tilePos.y];
    ...
    List<List<Vector2>> CalculateEdgePoints (Vector2Int startPos, ref List<Vector2Int> includedEdgeNodes, out int runIterations) {
        ...
        int startDir = 0;
        while (!GetNodeInMapAndFilled(startPos + GetDirFromDirectionIndex(startDir))) {
            startDir++;
            if (startDir > 3) {
        ...
        while (runIterations++ < 100000 && !shouldBreak) {
            ...
            if (!IsNodeInMap(curPos + curDirVector)) {
                ...
                if (!previousWasOutsideMap) {
                    ...
                    paths[pathIndex].Add(NodePosToWorldPos(mapPos));
                    previousWasOutsideMap = true;
                }
                curDir = RotateClockwise(curDir);
            } else if (GetNodeFilled(curPos + curDirVector)) {
                if (previousWasOutsideMap) {
                    ...
                    paths[pathIndex].Add(NodePosToWorldPos(rewindPos));
                }
